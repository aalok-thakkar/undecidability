...
4. **`starts_with_stepTiles`** — combines the copy and transition forcing
   ...
   * ✅ `starts_with_stepTilesRightBoundary` (under `qNew = some _`;
     the `qNew = none` halt-now branch is handled directly in
     `backward_aux` via `Halts` reachability).
   * ✅ `starts_with_stepTilesLeftInterior` (now unambiguous under the
     HUM one-sided-tape refactor — `leftMoveBoundaryTile` is no longer
     in `luTiles`, so the alternative `copyTile b :: leftMoveBoundaryTile`
     decomposition cannot be constructed).
   * The **left-boundary** case is *removed* by the HUM refactor:
     `NoLeftBoundary` (assumed on the input TM/word) ensures no
     reachable cfg ever invokes a left-move at the left boundary,
   ...
   * ✅ `no_tile_for_state_sharp` — if `tau1 A = ↟ₛq :: # :: rest` and
     `A ⊆ luTiles tm`, then `False` (every transition tile has a tape
     lift as its second character; all other tile families have a
     first character distinct from `↟ₛq`).
